...
const CreateJobPostPage = () => {
		...
		const fetchRecruiterInfo = async () => {
			try {
				...
				if (error || !recruiterData) {
					Alert.alert(
						"Profile Missing",
						"You need to complete your recruiter profile before posting a job."
					);
					setLoadingRecruiter(false);
					return;
				}
				...
				Alert.alert("Error", e.message || "Failed to load recruiter info.");
			} finally {
				setLoadingRecruiter(false);
			}
		};
	...
	// Helpers to generate unique keys for draggable lists
	...
	const handleSubmit = async () => {
		...
		const jobData = {
			...
			// created_at omitted, Supabase/Postgres will handle automatically
		};
		...
		return (
			...
			>
				...
				>
					...
					<Text style={{ marginTop: 10 }}>Loading recruiter profile...</Text>
				</View>
			</SafeAreaView>
		);
	}
	...
	if (!companyId) {
		return (
			...
			>
				...
				>
					You must complete your recruiter profile before posting jobs.
				</Text>
			</SafeAreaView>
		);
	}
